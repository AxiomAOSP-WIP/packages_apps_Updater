...
updates_dir=/data/axiomos_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have axiomos-VERSION-DATE-TYPE-* as format"
...
# Assume axiomos-VERSION-DATE-TYPE-*.zip
...
adb shell "killall com.axiomos.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.axiomos.updater/databases/updates.db" \
